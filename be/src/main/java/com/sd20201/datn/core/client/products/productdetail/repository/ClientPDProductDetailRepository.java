...
public interface ClientPDProductDetailRepository extends ProductDetailRepository {
    ...
    @Query(value = """
                ...
                GROUP BY
                    ...
                    p.material.name, p.product.brand.name, p.product.battery.name, 
                    p.product.operatingSystem.name
        """)
    Optional<ClientPDProductDetailDetailResponse> getProductVariant(@Param("req") LayThuocTinhRequest req, Long time);
}