...
import com.sd20201.datn.core.client.products.product.model.response.ClientProductDetailResponse;
import com.sd20201.datn.core.client.products.product.model.response.ClientProductResponse;
...
public interface ClientProductRepository extends ProductRepository {
    ...
    HAVING (:#{#request.minPrice} <= MIN(pd.price) AND MAX(pd.price) <= :#{#request.maxPrice})
    """)
    Page<ClientProductResponse> getProducts(Pageable pageable, ClientProductRequest request, Long date);
    ...
    WHERE id = :id
    """)
    Optional<ClientProductDetailResponse> getProductById(@Param("id") String id);
    ...
    ORDER BY p.createdDate DESC
    """)
    List<ClientPRPropertyComboboxResponse> getPropertyCombobox();
}
