...
public interface ClientTaoHoaDonChiTietRepository extends InvoiceDetailRepository {
    ...
    @Query(value = """
                SELECT
                    ...
                    CAST(
                        CASE 
                            WHEN d.id IS NOT NULL THEN spct.price * (100 - d.percentage) / 100
                            ELSE spct.price 
                        END AS DECIMAL(19,2)
                    ) AS giaSauGiam,
                    ...
                    g.name AS gpu,
                    cl.name AS color,
                    GROUP_CONCAT(i.code SEPARATOR ', ') AS imel
                ...
                GROUP BY hdct.id, spct.id, sp.name, hdct.quantity, spct.price, spct.url_image, 
                         c.name, r.name, hd2.name, g.name, cl.name, d.id, d.percentage, hdct.created_date
