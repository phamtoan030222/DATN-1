...
function handleLogin() {
  formRef.value?.validate(async (errors) => {
    ...
    try {
      const token = await postLogin(account, pwd, "CUSTOMER");
      if (token) {
...
const handleRedirectLoginCUSTOMER = () => {
  ...
  <div>
    ...
    <n-form ref="formRef" :rules="rules" :model="formValue" :show-label="false" size="large">
      <n-form-item path="account">
        <n-input v-model:value="formValue.account" clearable placeholder="Nhập tài khoản" />
      </n-form-item>
      <n-form-item path="pwd">
        <n-input v-model:value="formValue.pwd" type="password" placeholder="Nhập mật khẩu" clearable
    ...
    <n-space justify="center">
      <n-button circle @click="handleRedirectLoginCUSTOMER">
        <template #icon>
