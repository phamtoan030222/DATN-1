...
interface Props {
  show: boolean
  deliveryType: 'GIAO_HANG' | 'TAI_QUAY'
  loading: boolean
}
...
const rules = computed(() => {
  return {
    ...
    storeId: props.deliveryType === 'TAI_QUAY'
      ? { required: true, message: 'Vui lòng chọn siêu thị', trigger: ['blur', 'change'] }
      : { required: false },
  }
})
  ...
  >
    ...
    >
      ...
      <template v-else>
        <NFormItem label="Chọn siêu thị" path="storeId">
          <NSelect
            ...
            placeholder="Chọn siêu thị gần bạn"
          />
        </NFormItem>
      ...
      <NFormItem label="Ghi chú">
        <NInput v-model:value="formData.ghiChu" type="textarea" placeholder="Lời nhắn (không bắt buộc)" />
      </NFormItem>
