...
import { AlertCircleOutline, ArrowBack, CartOutline, CheckmarkCircle, Flash, RefreshOutline, TicketOutline } from '@vicons/ionicons5'

// Import API
import { getColors, getCPUs, getGPUs, getHardDrives, getImeiProductDetail, getProductDetailById, getProductDetails, getRAMs } from '@/service/api/admin/product/productDetail.api'
...
const allVariants = ref<any[]>([]) // Dùng cho bộ lọc thông minh
...
async function fetchData(id: string) {
  ...
  try {
    ...
    if (res.data) {
      ...
      await loadAllVariants(product.value.productName || product.value.name)
  ...
  catch (error) { message.error('Không thể tải thông tin sản phẩm') }
  finally { loading.value = false }
}

async function loadGlobalOptions() {
  const mapOpt = (arr: any) => (arr?.data ?? arr ?? []).map((x: any) => ({ label: x.label || x.name || x.code, value: x.value || x.id }))
  const [gpus, cpus, rams, hds, colors] = await Promise.all([getGPUs(), getCPUs(), getRAMs(), getHardDrives(), getColors()])
...
async function loadAllVariants(productName: string) {
  try {
    // Lấy tất cả biến thể cùng tên để so sánh logic
    const res = await getProductDetails({ page: 1, size: 100, keyword: productName })
    const list = res?.data?.content || res?.data?.data || []
    allVariants.value = list.filter((x: any) => (x.productName || x.product || x.name) === productName)
  }
  catch (e) { console.error(e) }
}

// --- 2. LOGIC BỘ LỌC THÔNG MINH (Smart Filter) ---
function isOptionDisabled(type: 'GPU' | 'CPU' | 'RAM' | 'HDD' | 'COLOR', value: string) {
  ...
  // Bỏ qua tiêu chí của chính loại đang check
  if (type === 'GPU')
  ...
  // Tìm xem có biến thể nào khớp không
  const match = allVariants.value.find((v) => {
    return (!criteria.idGPU || v.idGPU === criteria.idGPU)
  ...
  return !match // Không tìm thấy -> Disable
}

async function handleOptionClick() {
  const exactMatch = allVariants.value.find(v =>
...
function handleResetFilter() {
  selectedGpu.value = null; selectedCpu.value = null; selectedRam.value = null
  selectedHardDrive.value = null; selectedColor.value = null
  message.info('Đã làm mới bộ lọc')
}

// --- 3. AUTO IMEI & STOCK ---
async function loadImeis(productDetailId: string) {
  selectedImeiId.value = null; isOutOfStock.value = false; imeiList.value = []
  try {
    ...
    if (imeiList.value.length > 0) {
      selectedImeiId.value = imeiList.value[0].id // Auto select first
    }
    else {
...
function startCountdown() {
  ...
  if (!endDate) { timeLeft.value = ''; return }
  if (timerInterval)
    clearInterval(timerInterval)
  timerInterval = setInterval(() => {
    const now = new Date().getTime(); const distance = endDate - now
    if (distance < 0) { timeLeft.value = 'ĐÃ KẾT THÚC'; clearInterval(timerInterval); return }
...
function handleBuyNow() {
  if (isOutOfStock.value)
    return
  if (!selectedImeiId.value) { message.warning('Hệ thống đang chọn IMEI, vui lòng chờ...'); return }
  router.push({ path: '/checkout', query: { productId: product.value.id, imeiId: selectedImeiId.value } })
}
function handleAddToCart() { isOutOfStock.value ? null : message.success(`Đã thêm vào giỏ hàng`) }
      ...
      <NGrid x-gap="24" cols="1 m:2" responsive="screen">
        <NGridItem>
          <div class="gallery-box relative mb-8">
            <div class="main-image-wrapper border border-gray-100 rounded-lg overflow-hidden bg-white">
              ...
              <div v-if="currentPercent > 0" class="absolute top-4 left-4 bg-green-600 text-white font-bold px-3 py-1 rounded shadow-md z-10 animate-pulse">
                -{{ currentPercent }}%
              </div>
            </div>
          </div>
          <div class="config-table-section">
            ...
            <NDescriptions bordered label-placement="left" size="small" column="1" label-style="width: 140px; font-weight: 600; background-color: #f0fdf4; color: #166534;">
        ...
        <NGridItem>
          <div class="info-box">
            ...
            <div v-if="currentPercent > 0" class="flash-sale-bar bg-gradient-to-r from-green-600 to-teal-500 text-white p-3 rounded-t-lg flex justify-between items-center shadow-md select-none">
              <div class="flex items-center gap-2">
                <NIcon size="24" class="animate-pulse">
                  <Flash />
                </NIcon><span class="font-bold text-lg uppercase tracking-wide">ƯU ĐÃI ĐẶC BIỆT</span>
              </div>
              <div class="flex items-center gap-2 bg-black/20 px-3 py-1 rounded backdrop-blur-sm">
                <span class="text-xs opacity-90">Kết thúc trong:</span><span class="font-mono font-bold text-lg text-yellow-300">{{ timeLeft }}</span>
              </div>
            </div>

            <div class="price-section bg-green-50/50 p-5 border border-green-100 mb-6 transition-all" :class="{ 'rounded-b-lg border-t-0': currentPercent > 0, 'rounded-lg': currentPercent <= 0 }">
                ...
                <div v-if="gpuOptions.some(o => !isOptionDisabled('GPU', o.value))">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <button
                      ...
                      class="chip" :class="{ active: selectedGpu === opt.value, disabled: isOptionDisabled('GPU', opt.value) }"
                      @click="!isOptionDisabled('GPU', opt.value) && (selectedGpu = opt.value, handleOptionClick())"
                    >
                      {{ opt.label }}
                    </button>
                  </div>
                ...
                <div v-if="cpuOptions.some(o => !isOptionDisabled('CPU', o.value))">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <button
                      ...
                      class="chip" :class="{ active: selectedCpu === opt.value, disabled: isOptionDisabled('CPU', opt.value) }"
                      @click="!isOptionDisabled('CPU', opt.value) && (selectedCpu = opt.value, handleOptionClick())"
                    >
                      {{ opt.label }}
                    </button>
                  </div>
                ...
                <div v-if="ramOptions.some(o => !isOptionDisabled('RAM', o.value))">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <button
                      ...
                      class="chip" :class="{ active: selectedRam === opt.value, disabled: isOptionDisabled('RAM', opt.value) }"
                      @click="!isOptionDisabled('RAM', opt.value) && (selectedRam = opt.value, handleOptionClick())"
                    >
                      {{ opt.label }}
                    </button>
                  </div>
                ...
                <div v-if="hardDriveOptions.some(o => !isOptionDisabled('HDD', o.value))">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <button
                      ...
                      class="chip" :class="{ active: selectedHardDrive === opt.value, disabled: isOptionDisabled('HDD', opt.value) }"
                      @click="!isOptionDisabled('HDD', opt.value) && (selectedHardDrive = opt.value, handleOptionClick())"
                    >
                      {{ opt.label }}
                    </button>
                  </div>
                ...
                <div v-if="colorOptions.some(o => !isOptionDisabled('COLOR', o.value))">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <button
                      ...
                      class="chip" :class="{ active: selectedColor === opt.value, disabled: isOptionDisabled('COLOR', opt.value) }"
                      @click="!isOptionDisabled('COLOR', opt.value) && (selectedColor = opt.value, handleOptionClick())"
                    >
                      {{ opt.label }}
                    </button>
                  </div>
            ...
            <div class="voucher-section mb-6 border border-dashed border-green-300 bg-green-50 p-4 rounded-lg hover:bg-green-100/50 transition-colors">
              <div class="flex justify-between items-center">
                ...
                <NButton size="small" type="primary" ghost color="#059669" @click="showVoucherModal = true">
                  {{ selectedVoucher ? 'Đổi mã khác' : 'Chọn mã giảm giá' }}
                </NButton>
              </div>
              <div v-if="selectedVoucher" class="mt-3 bg-white border border-green-200 p-3 rounded flex justify-between items-center shadow-sm">
                <div>
                  <div class="font-bold text-green-600">
                    {{ selectedVoucher.code }}
                  </div>
                  <div class="text-sm text-gray-600">
                    Đã giảm: <span class="font-bold text-green-600">-{{ formatCurrency(voucherDiscountAmount) }}</span>
                  </div>
                </div>
                <NIcon color="#059669" size="24">
                  <CheckmarkCircle />
                </NIcon>
              </div>
            </div>
            ...
            <div class="actions border-t pt-6">
              <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-4">
                  ...
                  <NInputNumber v-model:value="quantity" :min="1" :max="10" button-placement="both" class="w-32" :disabled="isOutOfStock" />
                </div>
              ...
              <div class="flex gap-4 h-12">
                <NButton v-if="!isOutOfStock" type="primary" class="flex-1 h-full text-lg font-bold shadow-lg shadow-green-200 hover:-translate-y-0.5 transition-transform" color="#059669" @click="handleBuyNow">
                  MUA NGAY
                </NButton>
                ...
                <NButton strong secondary type="info" class="flex-1 h-full text-lg font-bold hover:-translate-y-0.5 transition-transform" :disabled="isOutOfStock" @click="handleAddToCart">
                  <template #icon>
    ...
    <NModal v-model:show="showVoucherModal" preset="card" title="Mã Giảm Giá Khả Dụng" style="width: 500px">
      ...
      <div v-else class="space-y-3 p-1 max-h-[400px] overflow-y-auto">
        <div
          ...
          :class="{ 'border-green-500 bg-green-50 ring-1 ring-green-200': selectedVoucher?.id === v.id, 'border-gray-200': selectedVoucher?.id !== v.id }"
          @click="handleSelectVoucher(v)"
        >
          <div v-if="index === 0" class="absolute -top-2 -right-2 bg-yellow-500 text-white text-[10px] px-2 py-0.5 rounded-full shadow-sm z-10 font-bold">
            TỐT NHẤT
          </div>
          ...
          <div class="flex justify-between items-start">
            <div class="flex-1">
              <div class="flex items-center gap-2">
                <span class="font-bold text-lg text-green-600 group-hover:text-green-700">{{ v.code }}</span>
                <NTag size="tiny" type="success" bordered>
            ...
            <div class="flex items-center justify-center h-full pl-3">
              <div
                ...
                :class="{ 'bg-green-500 border-green-500': selectedVoucher?.id === v.id }"
              >
...
<style scoped>
.detail-container { max-width: 1200px; margin: 0 auto; padding: 30px 20px; background-color: #fff; min-height: 80vh; }
.main-image-wrapper { position: relative; padding-top: 75%; }
.main-image-wrapper img { position: absolute; top: 0; left: 0; }
.chip { padding: 8px 12px; border: 1px solid #e5e7eb; border-radius: 8px; background: #fff; font-size: 12px; transition: all 0.2s; cursor: pointer; }
.chip:hover { border-color: #10b981; }
.chip.active { border-color: #059669; background: #ecfdf5; color: #059669; font-weight: 600; }
.chip.disabled { opacity: 0.4; background: #f3f4f6; border-color: #e5e7eb; color: #9ca3af; cursor: not-allowed; pointer-events: none; text-decoration: line-through; }
</style>
