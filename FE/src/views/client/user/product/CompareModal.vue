...
import { NModal, NSelect, NTag, useMessage, NIcon, NButton } from 'naive-ui'
import { SearchOutline, CartOutline } from '@vicons/ionicons5'
  ...
  searchTimeout = setTimeout(async () => {
    try {
      const res = await getProductDetails({
        page: 1, size: 20, q: query, minPrice: 0, maxPrice: 1000000000
      })
      ...
      searchOptions.value = list.map(item => ({
        label: item.name, value: item.id, data: item 
      }))
    } catch (error) {
...
<template>
  ...
  >
    <div class="compare-container" v-if="currentProduct">
      <table class="compare-table">
        <thead>
          <tr>
            <th class="prop-col">Thuộc tính</th>
            
            <th class="product-col">
              <div class="compare-header-info">
                <NTag type="success" size="small" style="margin-bottom: 8px; width: fit-content;">Máy hiện tại</NTag>
                <p class="compare-name">{{ currentProduct.name }}</p>
              </div>
            </th>
            ...
            <th class="product-col">
              <div class="compare-header-info">
                <NSelect
                  v-model:value="compareTargetId1"
                  :options="searchOptions"
                  placeholder="🔍 Gõ tên sản phẩm 1..."
                  filterable clearable remote
                  :loading="isSearching"
                  @search="handleSearch"
                  @update:value="handleSelectCompareItem1"
                >
                  <template #arrow><NIcon><SearchOutline /></NIcon></template>
                </NSelect>
            ...
            <th class="product-col">
              <div class="compare-header-info">
                <NSelect
                  v-model:value="compareTargetId2"
                  :options="searchOptions"
                  placeholder="🔍 Gõ tên sản phẩm 2..."
                  filterable clearable remote
                  :loading="isSearching"
                  @search="handleSearch"
        ...
        <tbody>
          <tr>
            ...
            <td class="center">
              ...
              <div v-else class="empty-placeholder"><span class="text-gray">Trống</span></div>
            </td>
            <td class="center">
          ...
          <tr>
            ...
            <td class="center">
              <div class="price-wrapper">
                <span v-if="currentProduct.percentage > 0" class="old-price">{{ formatCurrency(currentProduct.price) }}</span>
            ...
            <td class="center">
              <div v-if="selectedCompareProduct1" class="price-wrapper">
                <span v-if="selectedCompareProduct1.percentage > 0" class="old-price">{{ formatCurrency(selectedCompareProduct1.price) }}</span>
            ...
            <td class="center">
              <div v-if="selectedCompareProduct2" class="price-wrapper">
                <span v-if="selectedCompareProduct2.percentage > 0" class="old-price">{{ formatCurrency(selectedCompareProduct2.price) }}</span>
          ...
          <tr>
            ...
            <td class="center">
              <div v-if="currentProduct.percentage > 0" class="promo-box">
                <NTag type="error" size="small" round>🔥 Giảm {{ currentProduct.percentage }}%</NTag>
                <span v-if="currentProduct.endDate" class="promo-date">Đến: {{ formatDate(currentProduct.endDate) }}</span>
              </div>
              <span v-else class="text-gray">-</span>
            </td>
            <td class="center">
              <div v-if="selectedCompareProduct1 && selectedCompareProduct1.percentage > 0" class="promo-box">
                <NTag type="error" size="small" round>🔥 Giảm {{ selectedCompareProduct1.percentage }}%</NTag>
                <span v-if="selectedCompareProduct1.endDate" class="promo-date">Đến: {{ formatDate(selectedCompareProduct1.endDate) }}</span>
              </div>
              <span v-else class="text-gray">-</span>
            </td>
            <td class="center">
              <div v-if="selectedCompareProduct2 && selectedCompareProduct2.percentage > 0" class="promo-box">
                <NTag type="error" size="small" round>🔥 Giảm {{ selectedCompareProduct2.percentage }}%</NTag>
                <span v-if="selectedCompareProduct2.endDate" class="promo-date">Đến: {{ formatDate(selectedCompareProduct2.endDate) }}</span>
              </div>
              <span v-else class="text-gray">-</span>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>{{ selectedCompareProduct1 ? (selectedCompareProduct1.cpuName || selectedCompareProduct1.cpu || 'N/A') : '-' }}</td>
            <td>{{ selectedCompareProduct2 ? (selectedCompareProduct2.cpuName || selectedCompareProduct2.cpu || 'N/A') : '-' }}</td>
          </tr>
          <tr>
            <td>RAM</td>
            <td>{{ currentProduct.ramName || currentProduct.ram || 'N/A' }}</td>
            <td>{{ selectedCompareProduct1 ? (selectedCompareProduct1.ramName || selectedCompareProduct1.ram || 'N/A') : '-' }}</td>
            <td>{{ selectedCompareProduct2 ? (selectedCompareProduct2.ramName || selectedCompareProduct2.ram || 'N/A') : '-' }}</td>
          </tr>
          <tr>
            <td>Ổ cứng</td>
            <td>{{ currentProduct.hardDriveName || currentProduct.hardDrive || 'N/A' }}</td>
            <td>{{ selectedCompareProduct1 ? (selectedCompareProduct1.hardDriveName || selectedCompareProduct1.hardDrive || 'N/A') : '-' }}</td>
            <td>{{ selectedCompareProduct2 ? (selectedCompareProduct2.hardDriveName || selectedCompareProduct2.hardDrive || 'N/A') : '-' }}</td>
          </tr>
          <tr>
            <td>Card đồ hoạ (VGA)</td>
            <td>{{ currentProduct.gpuName || currentProduct.gpu || 'N/A' }}</td>
            <td>{{ selectedCompareProduct1 ? (selectedCompareProduct1.gpuName || selectedCompareProduct1.gpu || 'N/A') : '-' }}</td>
            <td>{{ selectedCompareProduct2 ? (selectedCompareProduct2.gpuName || selectedCompareProduct2.gpu || 'N/A') : '-' }}</td>
          </tr>
          <tr>
            <td>Màn hình</td>
            <td>{{ currentProduct.screenName || currentProduct.screen || 'N/A' }}</td>
            <td>{{ selectedCompareProduct1 ? (selectedCompareProduct1.screenName || selectedCompareProduct1.screen || 'N/A') : '-' }}</td>
            <td>{{ selectedCompareProduct2 ? (selectedCompareProduct2.screenName || selectedCompareProduct2.screen || 'N/A') : '-' }}</td>
          </tr>
          <tr>
            <td>Màu sắc</td>
            <td>{{ currentProduct.colorName || currentProduct.color || 'N/A' }}</td>
            <td>{{ selectedCompareProduct1 ? (selectedCompareProduct1.colorName || selectedCompareProduct1.color || 'N/A') : '-' }}</td>
            <td>{{ selectedCompareProduct2 ? (selectedCompareProduct2.colorName || selectedCompareProduct2.color || 'N/A') : '-' }}</td>
          </tr>
