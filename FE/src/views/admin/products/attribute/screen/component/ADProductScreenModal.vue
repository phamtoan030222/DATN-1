            ...
            <div :style="{ maxHeight: '400px', overflowY: 'auto' }">
                <n-form>
                    <n-form-item label="Mã">
                        <n-input v-model:value="detailScreen.code" placeholder="Nhập mã"></n-input>
                    </n-form-item>
                    <n-form-item label="Tên">
                        <n-input v-model:value="detailScreen.name" placeholder="Nhập tên"></n-input>
                    </n-form-item>
                    <n-form-item label="Tấm nền">
                        <n-select v-model:value="detailScreen.panelType" :options="panelTypeOptionsSelect"
                        :placeholder="'Chọn tấm nền'"></n-select>
                    </n-form-item>
                    <n-form-item label="Độ phân giải">
                        <n-select v-model:value="detailScreen.resolution" :options="screenResolutionOptionsSelect"
                        :placeholder="'Chọn kích thước màn hình'"></n-select>
                    </n-form-item>
                    <n-form-item label="Kích thước màn hình">
                        <n-select v-model:value="detailScreen.physicalSize" :options="screenSizeOptionsSelect"
                            :placeholder="'Chọn kích thước màn hình'"></n-select>
                    </n-form-item>
                    <n-form-item label="Công nghệ">
                        <n-input v-model:value="detailScreen.technology" placeholder="Nhập công nghệ"></n-input>
                    </n-form-item>
                </n-form>
            </div>
...
import { ADProductScreenDetailResponse, getScreenById, modifyScreen } from '@/service/api/admin/product/screen.api';
...
const handleClickOK = async () => {
    try {
        ...
        if (res.success) notification.success({ content: props.id ? 'Cập nhật màn hình thành công' : 'Thêm màn hình thành công', duration: 3000 })
