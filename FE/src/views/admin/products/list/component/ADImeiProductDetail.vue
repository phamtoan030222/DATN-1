...
async function clickAddIMEIHandler() {
  if (!imei.value) {
    return
  }
...
function handleClickOK() {
  ...
  if (props.idProductDetail) {
    emit('update:imei', { idProductDetail: props.idProductDetail, imeis })
  }
  else {
...
async function downloadIMEITemplate() {
  try {
...
async function handleUploadImportExcel({ file }: UploadCustomRequestOptions) {
  try {
...
<template>
  <n-modal :show="isOpen">
    <n-card
      style="width: 800px; height: 600px" title="Quản lý Serial biến thể sản phẩm" :bordered="false" size="huge" role="dialog"
      aria-modal="true"
    >
      ...
      <div :style="{ maxHeight: '400px', overflowY: 'auto' }">
        <n-flex>
          <n-input v-model:value="imei" style="flex: 1;" placeholder="Nhập giá trị serial" />
        </n-flex>
        <n-space class="mt-20px">
          <NButton @click="downloadIMEITemplate">
            Tải template serial
          </NButton>
      ...
      <template #footer>
        <n-space justify="end">
          ...
          <n-popconfirm
            ...
            @positive-click="handleClickOK" @negative-click="handleClickCancel"
          >
            <template #trigger>
