...
import type { ADProductCreateUpdateRequest, ADPRPropertiesComboboxResponse } from '@/service/api/admin/product/product.api'
import { getBatteries, getBrands, getOperatingSystems, getProductById, getScreens, modifyProduct } from '@/service/api/admin/product/product.api'
import type { ADProductDetailCreateUpdateRequest } from '@/service/api/admin/product/productDetail.api'
import { checkExistVariant, createProductVariant, getColors, getCPUs, getGPUs, getHardDrives, getMaterials, getRAMs, saveImage } from '@/service/api/admin/product/productDetail.api'
import { Icon } from '@iconify/vue'
import type { DataTableColumns, FormInst, FormRules, UploadFileInfo } from 'naive-ui'
import { NButton, NInput, NInputNumber, NSpace, NUpload } from 'naive-ui'
...
function initSelectPropertiesProduct() {
...
interface ADPRTableProductDetail {
  idColor: string
  idMaterial: string
  idCPU: string
  idGPU: string
...
function handleClickPriceTable(idColor: string, index: number) {
  isEditPriceInputTable.value = { idColor, index }
}

function handleEnterPrice(idColor: string, idx: number) {
  ...
  const indexIdColor = productDetails.map((productDetail, index) => ({ idColor: productDetail.idColor, index }))
    .filter(productDetail => productDetail.idColor === idColor)
    .map(indexObj => indexObj.index)[0]
...
function openModalIMEIProduct(idColor: string, index: number) {
  isOpenModalIMEIProduct.value = true
  indexRowDataImei.value = index
  idColorImei.value = idColor
}

function handleEmitClose() {
  isOpenModalIMEIProduct.value = false
}

function handleEmitUpdateImei(imeis: string[], idColorImei: string, index: number) {
  const indexIdColor = productDetails.map((productDetail, index) => ({ idColor: productDetail.idColor, index }))
    .filter(productDetail => productDetail.idColor === idColorImei)
    .map(indexObj => indexObj.index)[0]
...
async function submitVariantHandler() {
  if (!validateSubmitVariantHandler())
    return
...
const validateSubmitVariantHandler: () => boolean = () => {
  if (!formDataBasicRef?.value?.validate((error) => {
    if (error)
      notification.error({ content: error[0][0]?.message ?? 'Vui lòng điền đầy đủ thông tin cơ bản', duration: 3000 })
    return !!error
  })) {
    return false
  }
  ...
  if (!formDataVariantRef?.value?.validate((error) => {
    if (error)
      notification.error({ content: error[0][0]?.message ?? 'Vui lòng điền đầy đủ thông tin biến thể', duration: 3000 })
    return !!error
  })) {
    return false
  }
...
function handleChangeImageProduct(data: { fileList: UploadFileInfo[] }) {
  imageProduct.push(data.fileList[0].file)
}

function handleChangeProductDetail(data: { fileList: UploadFileInfo[] }, idColor: string) {
...
function clickAddQuickPropertiesHandler(typeData: ProductPropertiesType) {
  isOpenQuickAddModal.value = true
  dataQuickAdd.type = typeData
}

function closeQuickAddModalHandler() {
  isOpenQuickAddModal.value = false
}

function applyPriceCommonVariantHandler(idColor: string) {
...
function getNameColorById(idColor: string) {
...
function deleteProductDetail(idColor: string, index: number) {
  const indexIdColor = productDetails
    .map((productDetail, index) => ({ idColor: productDetail.idColor, index }))
    .filter(productDetail => productDetail.idColor === idColor)
    .map(indexObj => indexObj.index)[0]

  productDetails.splice(indexIdColor + index, 1)

  // ✅ Chỉ xóa state nếu không còn biến thể nào của màu này
  const remainingCount = productDetails.filter(pd => pd.idColor === idColor).length
  if (remainingCount === 0) {
  ...
  <div>
    <n-card title="Thông tin cơ bản">
        ...
        <n-form ref="formDataBasicRef" :rules="rulesDataBasic" :model="formDataBasic">
          <n-grid :span="24" :x-gap="36" :y-gap="8">
            <n-form-item-gi :span="24" label="Tên sản phẩm" path="name">
              <NInput v-model:value="formDataBasic.name" placeholder="Nhập tên sản phẩm" />
            </n-form-item-gi>
            <n-form-item-gi :span="12">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.BRAND)"
                  >
              ...
              <n-select
                v-model:value="formDataBasic.idBrand" filterable placeholder="Chọn hãng"
                :options="dataProperties.basicInformation.brands"
              />
            </n-form-item-gi>
            <n-form-item-gi :span="12">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.BATTERY)"
                  >
                    <Icon icon="material-symbols:add" />
                  </NButton>
                </NSpace>
              ...
              <n-select
                v-model:value="formDataBasic.idBattery" filterable placeholder="Chọn pin"
                :options="dataProperties.basicInformation.batteries"
              />
            </n-form-item-gi>
            <n-form-item-gi :span="12">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.SCREEN)"
                  >
                    <Icon icon="material-symbols:add" />
                  </NButton>
                </NSpace>
              ...
              <n-select
                v-model:value="formDataBasic.idScreen" filterable placeholder="Chọn màn hình"
                :options="dataProperties.basicInformation.screens"
              />
            </n-form-item-gi>
            <n-form-item-gi :span="12">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.OPERATING_SYSTEM)"
                  >
                    <Icon icon="material-symbols:add" />
                  </NButton>
                </NSpace>
              ...
              <n-select
                v-model:value="formDataBasic.idOperatingSystem" filterable
                placeholder="Chọn hệ điều hành"
                :options="dataProperties.basicInformation.operatingSystems"
              />
            </n-form-item-gi>
          </n-grid>
        </n-form>
      ...
      <div v-if="!idProduct" class="mt-40px">
        ...
        <NUpload
          :default-upload="false" list-type="image-card" multiple style="height: 100px;"
            ...
            <n-form-item-gi :span="12" path="idColor">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.COLOR)"
                  >
                    <Icon icon="material-symbols:add" />
                  </NButton>
                </NSpace>
              ...
              <n-select
                v-model:value="formDataVariant.idColor"
            ...
            <n-form-item-gi :span="12" path="idCpu">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.CPU)"
                  >
                    <Icon icon="material-symbols:add" />
                  </NButton>
                </NSpace>
              ...
              <n-select
                v-model:value="formDataVariant.idCpu" filterable placeholder="Chọn CPU"
                :options="dataProperties.variantInformation.cpus" multiple clearable
              />
            </n-form-item-gi>
            <n-form-item-gi :span="12" path="idGpu">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.GPU)"
                  >
                    <Icon icon="material-symbols:add" />
                  </NButton>
                </NSpace>
              ...
              <n-select
                v-model:value="formDataVariant.idGpu" filterable placeholder="Chọn GPU"
                :options="dataProperties.variantInformation.gpus" multiple clearable
              />
            </n-form-item-gi>
            <n-form-item-gi :span="12" path="idMaterial">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.MATERIAL)"
                  >
                    <Icon icon="material-symbols:add" />
                  </NButton>
                </NSpace>
              ...
              <n-select
                v-model:value="formDataVariant.idMaterial" filterable placeholder="Chọn chất liệu"
                :options="dataProperties.variantInformation.materials" multiple clearable
              />
            </n-form-item-gi>
            <n-form-item-gi :span="12" path="idRam">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.RAM)"
                  >
                    <Icon icon="material-symbols:add" />
                  </NButton>
                </NSpace>
              ...
              <n-select
                v-model:value="formDataVariant.idRam" filterable placeholder="Chọn RAM"
                :options="dataProperties.variantInformation.rams" multiple clearable
              />
            </n-form-item-gi>
            <n-form-item-gi :span="12" path="idHardDrive">
              <template #label>
                <NSpace class="w-full" justify="space-between">
                  ...
                  <NButton
                    tertiary
                    size="tiny" @click="clickAddQuickPropertiesHandler(ProductPropertiesType.HARD_DRIVE)"
                  >
                    <Icon icon="material-symbols:add" />
                  </NButton>
                </NSpace>
              ...
              <n-select
                v-model:value="formDataVariant.idHardDrive" filterable placeholder="Chọn ổ cứng"
                :options="dataProperties.variantInformation.hardDrives" multiple clearable
              />
            </n-form-item-gi>
            <n-form-item-gi :span="24">
              <NSpace>
                <NButton type="primary" @click="createVariant">
                  Tạo biến thể
                </NButton>
              </NSpace>
            </n-form-item-gi>
    ...
    <n-card
      ...
      v-if="productDetails && productDetails.length > 0" class="mt-20px"
    >
      <template #header>
        <NSpace justify="space-between">
          ...
          <NSpace>
            <NInputNumber
              ...
              placeholder="Nhập giá chung" clearable :format="formatCurrency" :parse="parseCurrency" @update:value="val => {
            ...
            <NButton
              type="success"
              @click="applyPriceCommonVariantHandler(productDetailList[0].idColor)"
            >
              Áp dụng
            </NButton>
          </NSpace>
      ...
      <n-data-table
        :data="productDetailList" :columns="columns" :bordered="false" :max-height="400" :pagination="{
    ...
    <n-card v-if="productDetails && productDetails.length > 0" class="mt-20px">
      ...
      <div class="images-variant">
        <n-card v-for="(productDetails, key) in partitionProductDetailsByColor" :key="key">
          <template #header>
            <NSpace align="center" justify="space-around" style="font-size: 12px;">
          ...
          <template #default>
            <div>
              <NUpload
                style="width: 100%;" :max="1" :default-upload="false" list-type="image-card"
    ...
    <NSpace v-if="productDetails && productDetails.length > 0" class="mt-20px">
      <NButton>Hủy</NButton>
      <n-popconfirm positive-text="Xác nhận" negative-text="Hủy" @positive-click="submitVariantHandler">
        <template #trigger>
          <NButton :loading="loadingCreateVariant" type="success">
            Xác nhận
          </NButton>
        </template>
    ...
    <ADImeiProductDetail
      :is-open="isOpenModalIMEIProduct" :id-color-imei="idColorImei" :index="indexRowDataImei"
      @close="handleEmitClose" @update:imei="handleEmitUpdateImei"
    />

    <QuickAddModal
      :is-open="isOpenQuickAddModal" :type="dataQuickAdd.type" @close="closeQuickAddModalHandler"
