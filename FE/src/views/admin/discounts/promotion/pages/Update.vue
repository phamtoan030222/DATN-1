...
import type { AppliedProductResponse, DiscountResponse, ProductDetailResponse, ProductResponse, UpdateDiscountRequest } from '@/service/api/admin/discount/discountApi'
import * as XLSX from 'xlsx'

function handleExportAppliedExcel() {
...
function getStatusType() { const s = getDiscountStatus(); return s === 'active' ? 'success' : s === 'upcoming' ? 'info' : 'default' }
function getStatusText() { const s = getDiscountStatus(); return s === 'active' ? 'Đang diễn ra' : s === 'upcoming' ? 'Sắp diễn ra' : 'Đã kết thúc' }
...
async function fetchProducts() {
  ...
  catch (e) { message.error('Lỗi tải danh sách SP') }
...
function resetUnappliedFilters() { filterUnapplied.name = ''; filterUnapplied.color = null; filterUnapplied.ram = null; filterUnapplied.hardDrive = null; filterUnapplied.gpu = null; filterUnapplied.cpu = null }
...
function resetAppliedFilters() { filterApplied.name = ''; filterApplied.color = null; filterApplied.ram = null; filterApplied.hardDrive = null; filterApplied.gpu = null; filterApplied.cpu = null }
...
async function fetchAndAddUnappliedDetails(productId: string) {
...
async function handleBulkRemove() {
  ...
  catch (e) { message.error('Lỗi khi gỡ nhiều') }
...
async function handleRemoveProduct(id: string) {
  if (!canUpdateProducts.value)
    return
  try { await removeProductsFromDiscount(id); message.success('Đã gỡ sản phẩm'); fetchAppliedProducts() }
  catch (e) { message.error('Lỗi khi gỡ') }
}

async function handleSubmit() {
  if (!canUpdateInfo.value)
    return
...
const unappliedProductColumns: DataTableColumns<ExtendedProductDetail> = [
  ...
  },
  {
    ...
    render: (r: any) => {
      ...
      return h('div', { style: 'font-size: 12px; line-height: 1.5;' }, specs.map(spec => h('div', {}, [
...
<template>
    ...
    <div class="products-section">
      <NCard title="Danh sách sản phẩm ">
        ...
        <div :style="!canUpdateProducts ? 'opacity: 0.6; pointer-events: none;' : ''">
          <NDataTable v-model:checked-row-keys="checkedProductKeys" :columns="productColumns" :data="paginatedRawProducts" :loading="loadingProducts" :row-key="(row) => row.id" :pagination="false" size="small" max-height="400px" />
        </div>
        <div v-if="filteredRawProducts.length > productPageSize" class="flex justify-end mt-4">
  ...
  <NCard v-if="unappliedProducts.length > 0" title="Danh sách Sản Phẩm Chi Tiết chưa áp dụng" style="margin-top: 16px;">
      ...
      <NGrid :x-gap="12" :y-gap="8" :cols="24">
        <NGridItem :span="5">
          <div class="filter-label">
            Tên SPCT
          </div><NInput v-model:value="filterUnapplied.name" placeholder="Tìm tên..." size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            Màu
          </div><NSelect v-model:value="filterUnapplied.color" :options="uniqueColorsUnapplied" placeholder="Màu" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            RAM
          </div><NSelect v-model:value="filterUnapplied.ram" :options="uniqueRamsUnapplied" placeholder="RAM" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            Ổ cứng
          </div><NSelect v-model:value="filterUnapplied.hardDrive" :options="uniqueHardDrivesUnapplied" placeholder="HDD" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            GPU
          </div><NSelect v-model:value="filterUnapplied.gpu" :options="uniqueGpusUnapplied" placeholder="GPU" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            CPU
          </div><NSelect v-model:value="filterUnapplied.cpu" :options="uniqueCpusUnapplied" placeholder="CPU" clearable size="small" />
        </NGridItem>
        <NGridItem :span="4" class="flex items-end" style="padding-bottom: 2px;">
          <NButton circle type="default" secondary size="small" @click="resetUnappliedFilters">
            <template #icon>
              <NIcon size="18">
    ...
    <div class="flex justify-between items-center mb-2">
      <NSpace>
        <NTag type="warning" size="small">
          Hiển thị: {{ filteredUnapplied.length }} | Đang chọn: {{ selectedUnappliedKeys.length }}
        </NTag>
        <NPopconfirm positive-text="Đồng ý" negative-text="Hủy" @positive-click="handleBulkApply">
          <template #trigger>
            <NButton type="primary" size="small" :disabled="selectedUnappliedKeys.length === 0 || !canUpdateProducts" :loading="bulkApplying">
              <template #icon>
                <NIcon><Icon icon="carbon:checkmark-outline" /></NIcon>
              </template> Áp dụng ngay ({{ selectedUnappliedKeys.length }})
            </NButton>
          </template>
          Áp dụng giảm giá cho các sản phẩm đã chọn?
        </NPopconfirm>
      </NSpace>
      <NPopconfirm positive-text="Xóa" negative-text="Hủy" @positive-click="clearUnapplied">
        <template #trigger>
          <NButton quaternary size="small">
            <template #icon>
              <NIcon><Icon icon="carbon:close" /></NIcon>
            </template> Xóa hết danh sách này
          </NButton>
        </template>
    ...
    <NDataTable v-model:checked-row-keys="selectedUnappliedKeys" :columns="unappliedProductColumns" :data="paginatedUnapplied" :row-key="(row) => row.id" :pagination="false" size="small" max-height="400" />
    <div class="flex justify-end mt-4">
      <NPagination :page="unappliedCurrentPage" :page-size="unappliedPageSize" :page-count="Math.ceil(filteredUnapplied.length / unappliedPageSize)" @update:page="(p) => unappliedCurrentPage = p" />
    </div>
  </NCard>
  ...
  <NCard title="Danh sách Sản Phẩm Chi Tiết đã áp dụng" style="margin-top: 16px;">
    <div class="filter-container mb-4">
      <NGrid :x-gap="12" :y-gap="8" :cols="24">
        <NGridItem :span="5">
          <div class="filter-label">
            Tên sản phẩm
          </div><NInput v-model:value="filterApplied.name" placeholder="Tìm tên ...." size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            Màu
          </div><NSelect v-model:value="filterApplied.color" :options="uniqueColorsApplied" placeholder="Màu" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            RAM
          </div><NSelect v-model:value="filterApplied.ram" :options="uniqueRamsApplied" placeholder="RAM" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            Ổ cứng
          </div><NSelect v-model:value="filterApplied.hardDrive" :options="uniqueHardDrivesApplied" placeholder="HDD" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            GPU
          </div><NSelect v-model:value="filterApplied.gpu" :options="uniqueGpusApplied" placeholder="GPU" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            CPU
          </div><NSelect v-model:value="filterApplied.cpu" :options="uniqueCpusApplied" placeholder="CPU" clearable size="small" />
        </NGridItem>
        <NGridItem :span="4" class="flex items-end" style="padding-bottom: 2px;">
          <NButton circle type="default" secondary size="small" @click="resetAppliedFilters">
            <template #icon>
              <NIcon size="18">
                <Icon icon="carbon:rotate-360" />
              </NIcon>
    ...
    <template #header-extra>
      <NSpace>
        <NPopconfirm positive-text="Gỡ bỏ" negative-text="Hủy" @positive-click="handleBulkRemove">
          <template #trigger>
            <NButton type="error" secondary :disabled="selectedAppliedKeys.length === 0 || !canUpdateProducts" :loading="bulkRemoving">
              ...
              </template> Gỡ bỏ ({{ selectedAppliedKeys.length }})
            </NButton>
          </template>
    ...
    <NDataTable v-model:checked-row-keys="selectedAppliedKeys" :columns="appliedProductColumns" :data="paginatedApplied" :loading="loadingApplied" :row-key="(row) => row.id" :pagination="false" max-height="400px" />
    <div class="flex justify-end mt-4">
      <NPagination :page="appliedCurrentPage" :page-size="appliedPageSize" :page-count="Math.ceil(filteredAppliedProducts.length / appliedPageSize)" @update:page="(p) => appliedCurrentPage = p" />
    </div>
  </NCard>
</template>
