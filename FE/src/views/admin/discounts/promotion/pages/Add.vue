...
import type { CreateDiscountRequest, ProductDetailResponse, ProductResponse } from '@/service/api/admin/discount/discountApi'
...
function resetFilters() { filterState.name = ''; filterState.color = null; filterState.ram = null; filterState.hardDrive = null; filterState.gpu = null; filterState.cpu = null }
...
async function fetchProducts() {
  ...
  catch (error) { message.error('Lỗi tải danh sách sản phẩm') }
...
async function fetchAndAddDetails(productId: string) {
  ...
  catch (e) { message.error('Lỗi tải biến thể sản phẩm') }
  finally { loadingProductDetails.value = false }
}
...
async function handleSubmit() {
  ...
  try {
    ...
    if (!createRes?.data?.id)
      throw new Error('Lỗi hệ thống: Không có ID')
...
const productDetailColumns: DataTableColumns<ExtendedProductDetail> = [
  ...
  },
  {
    ...
    render: (r: any) => {
      ...
      return h('div', { style: 'font-size: 12px; line-height: 1.5;' }, specs.map(spec =>
        h('div', {}, [
...
<template>
  ...
  <div class="main-layout">
        ...
        <NForm ref="formRef" :model="formData" :rules="formRules">
          <NSpace vertical :size="16">
            ...
            <NSpace justify="space-between" style="margin-top: 24px;">
              ...
              <NPopconfirm positive-text="Đồng ý" negative-text="Hủy" @positive-click="handleSubmit">
                <template #trigger>
    ...
    <div class="products-section">
      <NCard title="Danh sách sản phẩm ">
        ...
        <div v-if="checkedProductKeys.length > 0" class="flex justify-between items-center mb-2">
        ...
        />
        <div v-if="filteredProducts.length > 0" class="flex justify-end mt-4">
  ...
  <NCard v-if="selectedProductDetails.length > 0" title="Danh sách Sản Phẩm Chi Tiết sẽ áp dụng" style="margin-top: 16px;">
    <div class="filter-container mb-4">
      <NGrid :x-gap="12" :y-gap="8" :cols="24">
        <NGridItem :span="5">
          <div class="filter-label">
            Tên SP
          </div><NInput v-model:value="filterState.name" placeholder="Tìm tên..." clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            Màu sắc
          </div><NSelect v-model:value="filterState.color" :options="uniqueColors" placeholder="Màu" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            RAM
          </div><NSelect v-model:value="filterState.ram" :options="uniqueRams" placeholder="RAM" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            Ổ cứng
          </div><NSelect v-model:value="filterState.hardDrive" :options="uniqueHardDrives" placeholder="Ổ cứng" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            GPU
          </div><NSelect v-model:value="filterState.gpu" :options="uniqueGpus" placeholder="GPU" clearable size="small" />
        </NGridItem>
        <NGridItem :span="3">
          <div class="filter-label">
            CPU
          </div><NSelect v-model:value="filterState.cpu" :options="uniqueCpus" placeholder="CPU" clearable size="small" />
        </NGridItem>
        <NGridItem :span="4" class="flex items-end" style="padding-bottom: 2px;">
          <NButton circle type="default" secondary title="Làm mới bộ lọc" @click="resetFilters">
            <template #icon>
              <NIcon size="18">
    ...
    <div class="flex justify-end mt-4">
      <NPagination v-model:page="detailCurrentPage" :page-size="detailPageSize" :page-count="Math.ceil(filteredDetails.length / detailPageSize)" />
    </div>
  </NCard>
</template>
